        ...
        if highlight:
            if aux_index == None:
                for ytick, xtick in np.ndindex(pivot_df.shape):
                    ...
                    if (target * (1.0 - main_scaled_factor) <= value) and (value <= target * (1.0 + main_scaled_factor)):
                ...
                for ytick, xtick in np.ndindex(pivot_df.shape):
                    ...
                    # print(qcs[xtick], ds[ytick], value)
                    target = E * 256/ds[ytick] * qcs[xtick] / 21
                    if (target * (1.0 - main_scaled_factor) <= value) and (value <= target * (1.0 + main_scaled_factor)) and (aux_value <= aux_th):
                        rect = patches.Rectangle(
                            (xtick+0.05, ytick+0.05), 1-0.05*2, 1-0.05*2,
                            linewidth=2, edgecolor=markers_palette[i],
                            alpha=0.25,
...
if __name__ == "__main__":
    # # TPS
    # draw(-3, op="max", title="tps")
    # draw(-3, op="min", title="tps")
    draw(-3, op="avg", title="tps")
    # draw(-3, op="med", title="tps")
    ...
    # Latency
    # draw(-1, op="max", title="latency")
    # draw(-1, op="min", title="latency")
    draw(-1, op="avg", title="latency", highlight=False)
    draw(-1, op="avg", title="latency_highlight", highlight=True)
    draw(-1, aux_index=3, op="avg", title="latency_timeout", highlight=False)
    draw(-1, aux_index=3, op="avg",
         title="latency_timeout_highlight", highlight=True)
    # draw(-1, op="med", title="latency")
